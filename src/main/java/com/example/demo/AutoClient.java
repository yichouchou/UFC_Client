                ...
                new Thread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        try {
                            ...
                            HtmlAnchor elementById = (HtmlAnchor) htmlPage
                                    .getElementById("formatSelector" + (next + 1));
                            try {
                                elementById.click();
...
//        List<Object> byXPath = htmlPage.getByXPath("//*[@id=\"event2029\"]/div[2]/div[3]/table/tbody/tr[1]/td[16]/a/span/span");
                                ...
                                for (int k = 0; k < elements.size(); k++) {
                                    ...
                                    // 获得了比赛赛事
                                    String fightName = elements.get(k).getElementsByClass("table-header").first()
                                            .getElementsByTag("a").first().text();
                                    String fightDate = "";
                                    try {
                                        // 获得了比赛日期--注意，这里日期可能为空，也就是没有span标签，会获取失败，做异常判断，如果是异常，则日期为空
                                        fightDate = elements.get(k).getElementsByClass("table-header").first()
                                    ...
                                    Elements thead = elements.get(k).getElementsByTag("thead").get(1).select("tr")
                                            .get(0).select("th");
...
//            Elements elementsByOdd = elements.get(k).getElementsByClass("table-scroller");
                                    ...
                                    for (int i = 0; i < elementsByClass.size(); i++) {
                                        ...
                                        Elements trElements = elementsByClass.get(i).getElementsByTag("tbody").get(i)
                                                .getElementsByTag("tr");
                                        ...
                                        for (int e = 0; e < envenIndexList.size(); e++) {
                                            ...
                                            String peopleAStr = trElements.get(envenIndexList.get(e)).select("span")
                                                    .first().text();
                                            String peopleBStr = trElements.get(envenIndexList.get(e) + 1).select("span")
                                                    .first().text();
                                            ...
                                            Elements oddselectTD = trElements.get(envenIndexList.get(e) + 1)
                                                    .select("td");
                                            ...
                                            if (e == envenIndexList.size() - 1) {
                                                ...
                                                for (int z = 0; z < lastLength; z = z + 2) {
                                                    if (z == 0) {
                                                        ...
                                                        lastgambling.setMaybeResultTypeToA(
                                                                fighterA.getFighterName() + " will win");
                                                        lastgambling.setMaybeResultTypeToB(
                                                                fighterB.getFighterName() + " will lose");
                                                        lastgambling.setOddsList(new ArrayList<>());
                                                        Elements lasttdList = trElements.get(envenIndexList.get(e))
                                                                .select("td");
                                                        ...
                                                        try {
                                                            gamblingIn.setMaybeResultTypeToA(
                                                                    fighterA.getFighterName() + " "
                                                                            + trElements
                                                                                    .get(envenIndexList.get(e) + z - 1)
                                                                                    .select("th").first().text());
                                                        } catch (Exception e0) {
                                                            System.out.println("A越界" + (envenIndexList.get(e) + z - 1));
                                                        }

                                                        try {
                                                            gamblingIn.setMaybeResultTypeToB(fighterB.getFighterName()
                                                                    + "  " + trElements.get(envenIndexList.get(e) + z)
                                                                            .select("th").first().text());
                                                        } catch (Exception e2) {
                                                        ...
                                                        Elements prTD = trElements.get(envenIndexList.get(e) + z)
                                                                .select("td");
                                                        Elements prOddTD = trElements.get(envenIndexList.get(e) + z + 1)
                                                                .select("td");
                                                ...
                                                for (int r = 0; r < length - 1; r = r + 2) {
                                                    if (r == 0) {
                                                        ...
                                                        gambling.setMaybeResultTypeToA(
                                                                fighterA.getFighterName() + " will win");
                                                        gambling.setMaybeResultTypeToB(
                                                                fighterB.getFighterName() + " will lose");
                                                        gambling.setOddsList(new ArrayList<>());
                                                        Elements tdList = trElements.get(envenIndexList.get(e))
                                                                .select("td");
                                                        ...
                                                        Gambling gamblingIn = new Gambling();
                                                        try {
                                                            gamblingIn.setMaybeResultTypeToA(fighterA.getFighterName()
                                                                    + " " + trElements.get(envenIndexList.get(e) + r)
                                                                            .select("th").first().text());
                                                        } catch (Exception e0) {
                                                        ...
                                                        try {
                                                            gamblingIn.setMaybeResultTypeToB(
                                                                    fighterB.getFighterName() + "  "
                                                                            + trElements
                                                                                    .get(envenIndexList.get(e) + r + 1)
                                                                                    .select("th").first().text());
                                                        } catch (Exception e2) {
                                                        ...
                                                        Elements prTD = trElements.get(envenIndexList.get(e) + r)
                                                                .select("td");
                                                        Elements prOddTD = trElements.get(envenIndexList.get(e) + r + 1)
                                                                .select("td");
